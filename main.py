...
class GameWidget(Widget):
    ...
    def draw_all(self):
        ...
        with self.canvas:
            ...
            for exp in self.explosions:
                ...
                Color(1, 0.86, 0.16, 1)
                r2 = max(1, r // 2)
                Ellipse(pos=(exp[0] - r2, exp[1] - r2), size=(r2 * 2, r2 * 2))
            self._draw_hud(w, h)
    ...
    def _draw_player(self):
        ...
        cx = p[0] + p[2] / 2
        Color(0.16, 0.71, 1, 1)
        Triangle(points=[cx, p[1] + p[3], p[0], p[1], p[0] + p[2], p[1]])
        Color(0.47, 0.86, 1, 1)
        Ellipse(pos=(cx - dp(8), p[1] + p[3] - dp(28)), size=(dp(16), dp(16)))
        Color(1, 0.5, 0.1, 1)
        Triangle(points=[cx - dp(8), p[1], cx + dp(8), p[1], cx, p[1] - dp(15)])
...
class GameRoot(FloatLayout):
    def __init__(self, **kwargs):
        ...
        self.title_label = Label(text='OSTAD NADERI', font_size=dp(28),
            color=(1, 1, 0, 1), size_hint=(1, None), height=dp(40),
            pos_hint={'x': 0, 'top': 1})
        self.subtitle = Label(text='Amouzesh Barnameh Nevisi', font_size=dp(14),
            color=(0.5, 0.86, 1, 1), size_hint=(1, None), height=dp(30),
            pos_hint={'x': 0, 'top': 0.95})
        self.welcome = Label(text='Salam be bazi-e sefine-ye Ostad Naderi khosh oomadi',
            font_size=dp(13), color=(1, 1, 1, 1), size_hint=(1, None),
            height=dp(30), pos_hint={'x': 0, 'top': 0.45})
        self.instruction = Label(text='Baraye shoroo-e bazi dokmeye paeen ro bezan',
            font_size=dp(13), color=(0.5, 0.86, 1, 1), size_hint=(1, None),
            height=dp(28), pos_hint={'x': 0, 'top': 0.41})
        self.start_btn = Button(text='SHOROO-E BAZI', font_size=dp(18),
            background_color=(0.16, 0.71, 1, 1), color=(1, 1, 1, 1),
            size_hint=(None, None), size=(dp(280), dp(70)),
            pos_hint={'center_x': 0.5, 'y': 0.30})
        self.gameover_label = Label(text='GAME OVER', font_size=dp(36),
            color=(1, 0.24, 0.27, 1), size_hint=(1, None), height=dp(60),
            pos_hint={'x': 0, 'top': 0.85}, opacity=0)
        self.msg_label = Label(text='Entezar-e Ostad Naderi azat bishtar bood\ndobareh talash kon!',
            font_size=dp(16), color=(1, 0.51, 0.11, 1), size_hint=(1, None),
            height=dp(70), pos_hint={'x': 0, 'top': 0.75}, opacity=0)
        self.restart_btn = Button(text='SHOROO-E DOBAREH', font_size=dp(16),
            background_color=(0.2, 0.9, 0.47, 1), color=(0, 0, 0, 1),
            size_hint=(None, None), size=(dp(280), dp(65)),
            pos_hint={'center_x': 0.5, 'y': 0.15}, opacity=0)
        for w in [self.subtitle, self.title_label, self.welcome, self.instruction,
                  self.start_btn, self.gameover_label, self.msg_label, self.restart_btn]:
            self.add_widget(w)
    ...
    def _update_status(self, dt):
        ...
        if g.game_started and not g.game_over:
            for w in [self.welcome, self.instruction, self.title_label,
                      self.subtitle, self.start_btn, self.gameover_label,
                      self.msg_label, self.restart_btn]:
                w.opacity = 0
        elif g.game_over:
            for w in [self.gameover_label, self.msg_label, self.restart_btn]:
                w.opacity = 1
            for w in [self.start_btn, self.welcome, self.instruction,
                      self.title_label, self.subtitle]:
                w.opacity = 0
        else:
            for w in [self.welcome, self.instruction, self.title_label,
                      self.subtitle, self.start_btn]:
                w.opacity = 1
            for w in [self.gameover_label, self.msg_label, self.restart_btn]:
                w.opacity = 0
